...
for i in range(0,len(element_connections)):
    ...
    elements.append(Element(E[i],A[i],nodes,element_connections[i]))
...
#------------All vectors should now have compatible dimensions for solution using Jacobi method------------------------#

#Iteratively calculate the matrix solution {uses calcJacobian function located in Jacobian.py}
result_vector = calcJacobian(trimmedMatrix,dispVector,forceVector)
